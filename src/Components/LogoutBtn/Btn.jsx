...
import { getAuth, signOut } from "firebase/auth"; // استيراد دالة signOut من Firebase

const Btn = () => {
  const navigate = useNavigate();
  const auth = getAuth(); // الحصول على حالة المصادقة
  ...
  const popup = async () => {
    ...
    if (result.isConfirmed) {
      try {
        // تسجيل الخروج باستخدام Firebase
        await signOut(auth);

        // حذف التوكن والمستخدم من الكوكيز
